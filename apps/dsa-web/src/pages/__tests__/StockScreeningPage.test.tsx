...
describe('StockScreeningPage', () => {
  ...
  it('re-syncs enabled state when AlphaSift availability check fails after config is enabled', async () => {
    ...
    enableAlphaSift.mockRejectedValueOnce(new Error('AlphaSift 适配层不可用。请执行 uv sync --frozen'));
    ...
    expect(screen.getByText('AlphaSift 适配层不可用。请执行 uv sync --frozen')).toBeInTheDocument();
  });
